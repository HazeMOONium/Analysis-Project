...
\title{Warehouse Planning: Greedy Baseline and Enhanced Heuristic}
\author{Hazem \& Yahya}
\date{}
...
\section{Complexity and Space}
Let $V$ be nodes, $E$ edges, $M$ orders, $P$ pickers, $B$ batches, and $S$ shelves per batch.
\begin{itemize}[leftmargin=1.25em]
    \item Time: Floyd--Warshall $O(V^3)$; sorting $O(M \log M)$; batch selection worst-case $O(M^2)$; routing $O(B S^2)$. In practice, $S$ is small and batching is near-linear for typical constraints.
    \item Space: $O(V^2)$ for distances, $O(M)$ for orders, and $O(BS)$ for batch routes.
\end{itemize}
